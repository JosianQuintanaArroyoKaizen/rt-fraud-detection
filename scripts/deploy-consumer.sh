...
STACK_NAME="${ENVIRONMENT}-rtfd-lambda-consumer"
FOUNDATION_STACK="${ENVIRONMENT}-rtfd-streaming-foundation"

echo -e "${YELLOW}Deploying Consumer Lambda${NC}"

./scripts/package-consumer-lambda.sh

echo -e "${YELLOW}Deploying Lambda infrastructure...${NC}"
aws cloudformation deploy \
    --template-file cloudformation/03-lambda-consumer.yaml \
...
S3_KEY="lambda-deployments/consumer-lambda-$(date +%s).zip"
aws s3 cp build/consumer-lambda.zip s3://${DEPLOYMENT_BUCKET}/${S3_KEY} \
    --region ${REGION}
...
echo -e "${GREEN}Consumer Lambda deployed successfully${NC}"
